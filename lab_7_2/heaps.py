...
class MinHeap(Heap):
    ...
    def _sift_down(self, index):
        ...
        if (index * 2) <= len(self):
            ...
            if right > len(self):
                ...
                smallest = left if self._items[left] < self._items[right] else right
    ...
    def validate(self):
        ...
        heap_i = 1
        items_len = len(self._items)
        while heap_i < items_len:
            temp_root = self._items[heap_i]
            if heap_i*2 < items_len and not self._items[heap_i*2] > temp_root:
                return False
            if heap_i*2+1 < items_len and not self._items[heap_i*2+1] > temp_root:
                return False
            heap_i += 1
        return True
        # ===end student section===
