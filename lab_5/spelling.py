...
class ChainingHashTable():
    ...
    ***** DON'T worry you will fix this later:)
    ...
    def _hash(self, item):
        ...
        hashed = nice_hash(item)
        return hashed % self.n_slots
    ...
    def __contains__(self, item):
        ...
        return True if item in self._data[self._hash(item)] else False
        # ===end student section===
...
class LinearHashTable():
    ...
    def store(self, item):
        ...
        if not self._data[hashed]:
            self._data[hashed] = item
        else:
            i = (hashed + 1) % self.n_slots
            while i != hashed:
                if not self._data[i]:
                    self._data[i] = item
                    i = hashed
...
class QuadraticHashTable():
    ...
    def store(self, item):
        ...
        if not self._data[hashed]:
            ...
            while not inserted:
                hashed = self._next_free_slot(hashed)
                if not self._data[hashed]:
                    self._data[hashed] = item
                    inserted = True
        # ===end student section===
...
if __name__ == '__main__':
    ...
    # spellcheck_with_hashtable(doc_word_list, dictionary_word_list, 'Chaining', 1000)
    # NOTE:
